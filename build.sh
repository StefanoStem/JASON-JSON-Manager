...
# Release guard: fail if debug ingest endpoints or telemetry markers are present (skip when DEBUG=1)
if [ -z "$DEBUG" ]; then
  TELEMETRY_MARKERS=(
    '127.0.0.1:7244'
    '/ingest/'
...
if [ -n "$DEBUG" ]; then
  sed -i "s/connect-src 'none'/connect-src 'self' http:\/\/127.0.0.1:7244/" "$BUILD/chrome/manifest.json"
fi
cp "$ROOT/backgrounds/chrome.js"  "$BUILD/chrome/background.js"
for f in "${SHARED[@]}"; do cp -r "$ROOT/$f" "$BUILD/chrome/"; done
[ -f "$ROOT/debug.html" ] && cp "$ROOT/debug.html" "$BUILD/chrome/"
echo "  → build/chrome/"
...
if [ -n "$DEBUG" ]; then
  sed -i "s/connect-src 'none'/connect-src 'self' http:\/\/127.0.0.1:7244/" "$BUILD/firefox/manifest.json"
fi
...
# Manifest sync check: fail if build manifests diverge from source (skip when DEBUG=1)
if [ -z "$DEBUG" ]; then
